...
(function () {
  ...
  function logTriggerSnapshot(currentJob) {
    ...
    const parts = [
      ...
      `GM=${gm ? `@${gm.requestedAt || '?'} azId=${gm.azId || '(empty)'}` : 'none'}`,
      `LS=${ls ? `@${ls.requestedAt || '?'} azId=${ls.azId || '(empty)'}` : 'none'}`,
  ...
  function tick() {
    ...
    if (!stageReady && directTriggerReady) {
      ...
      log(`Recovered quote-grabber stage from direct trigger (source=${src})`);
    }
