// ==UserScript==
// @name         AZ TO GWPC 01 AZ Stage Runner + AZ Payload Grabber V2.0
// @namespace    home.bot.az.stage.runner.payload
// @version      2.0
// @description  Manual-start AZ stage runner. Pick Auto/Home, then Start. Uses visible AZ dock side-actions as the open signal, forces Main first, requires all 13 AZ fields before continuing, then clicks Quotes exactly, clicks Auto/Home, pauses in background, resumes after 3s in front, and only moves on when Bot Quoted appears on that same ticket.
...
(function () {
  ...
  function startRun() {
    ...
    log(`Started | mode=${state.mode.toUpperCase()}`, 'ok');
  ...
  function saveSharedJob(payload) {
    ...
    try { GM_setValue(KEYS.SHARED_JOB, shared); } catch (err) {
      log(`Shared save failed: ${err?.message || err}`, 'error');
    }

    try { GM_setValue(KEYS.CURRENT_JOB, currentJob); } catch (err) {
      log(`Current job save failed: ${err?.message || err}`, 'error');
    }
  }
