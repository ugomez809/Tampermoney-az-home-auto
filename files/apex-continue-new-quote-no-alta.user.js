// ==UserScript==
// @name         APEX Home Quote Continue No Alta
// @namespace    homebot.apex-continue-new-quote-no-alta
// @version      1.8.12
// @description  No Alta checkbox version. Detect Personal Lines Quote modal, click the real Home control that owns custom107, select Residence Address, wait for Continue New Quote readiness, and recover when one PC blocks the GWPC popup handoff.
// @author       OpenAI
// @match        https://farmersagent.lightning.force.com/*
// @run-at       document-idle
// @grant        none
// @updateURL    https://raw.githubusercontent.com/ugomez809/Tampermoney-az-home-auto/main/files/apex-continue-new-quote-no-alta.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/Tampermoney-az-home-auto/main/files/apex-continue-new-quote-no-alta.user.js
// ==/UserScript==
...
(function () {
  ...
  const SCRIPT_NAME = 'APEX Home Quote Continue No Alta';
  ...
  const LOG_PERSIST_KEY = 'tm_apex_continue_new_quote_no_alta_logs_v1';
  ...
  const CFG = {
    ...
    panelWidth: 340,
    zIndex: 2147483647,
    posKey: 'tm_apex_continue_new_quote_no_alta_panel_pos_v18',
    doneThisLoadKey: 'tm_apex_continue_new_quote_no_alta_done_this_load_v18',
  ...
  function getPanel() {
    return document.getElementById('hb-apex-continue-no-alta-panel');
  }
  ...
  function makePanel() {
    if (document.getElementById('hb-apex-continue-no-alta-panel')) return;

    const panel = document.createElement('div');
    panel.id = 'hb-apex-continue-no-alta-panel';
    panel.innerHTML = `
      <div id="hb-apex-continue-no-alta-drag" style="font-weight:700;margin-bottom:6px;cursor:move;display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <span>${SCRIPT_NAME}</span>
        <span style="font-size:11px;opacity:.8;">${VERSION}</span>
      </div>

      <div style="margin-bottom:6px;">
        <span id="hb-apex-continue-no-alta-status" style="font-size:12px;">Running</span>
      </div>

      <div>
        <div id="hb-apex-continue-no-alta-log" style="max-height:180px;overflow:auto;font-size:11px;line-height:1.35;"></div>
      </div>
    `;
    ...
    const dragHandle = document.getElementById('hb-apex-continue-no-alta-drag');
  ...
  function setStatus(text) {
    const el = document.getElementById('hb-apex-continue-no-alta-status');
  ...
  function log(msg) {
    ...
    const box = document.getElementById('hb-apex-continue-no-alta-log');
    if (!box) return;
  ...
  function hasVisibleLoadingIndicator(scope = document) {
    ...
    return candidates.some(el => {
      ...
      if (el.closest?.('#hb-apex-continue-no-alta-panel')) return false;
      return true;
    });
  }
